 ...
 * 	INA226.cpp
 *
 *	Class implementation for chip INA226
 *
		...
		const VBUSCT_VSHCT shunt_v_ct, Mode mode)
{
	...
	if ( avg_smpls != Avg_samples::as_is ) {
		reg_curr = ( reg_curr & Cfg_avg_samples_mask ) | ( static_cast<uint16_t>( avg_smpls ) << Cfg_avg_samples_lmove ) ;
	}
...
void INA226::Set_alert_func(Alert_func func, float value, bool latch_enable,
